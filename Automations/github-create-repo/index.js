...
const ask_project_location = () => {
	return new Promise((resolve, reject) => {
		rl.question(
			'In which directory you want to create your project (Please enter full absolute path) ? \n ',
			(answer) => {
...
;(async () => {
	try {
		...
		await page.click(
			'.details-overlay.details-reset.js-feature-preview-indicator-container > summary > img',
		)
		await page.waitForSelector('.dropdown-item.dropdown-signout')
		await page.click('.dropdown-item.dropdown-signout')
		await page.waitForNavigation({ timeout: 0 })
		await exec(
			`cd ~/${projectLocation}${projectName} && echo "# ${projectName}" >> README.md && git init && git add README.md && git commit -m "initial commit" && git branch -M main && git remote add origin https://github.com/${userName}/${projectName}.git && git push -u origin main`,
		)
